...
namespace VisualDebugger
{
	namespace Renderer
	{
		...
		void InitWindow(const char *name, int width, int height)
		{
			...
			int halfScreenWidth = glutGet(GLUT_SCREEN_WIDTH) / 2;
			int halfScreenHeight = glutGet(GLUT_SCREEN_HEIGHT) / 2;
			glutInitWindowSize(halfScreenWidth, halfScreenHeight);
		...
		void Render(PxActor** actors, const PxU32 numActors)
		{
			...
			{
				// dont render collision shape
				string actorName = actors[i]->getName();
				if (actorName == "GOALCOLLISION")
				{
					continue;
				}
